...
async function bgRefreshState(){
 ...
 if(key!==bgSeenRoll){
 bgSeenRoll=key;
 const dice=st.last_roll.map(d=>[1,2,3,4,5,6][d-1]||d).join('');
...
async function bgShowGame(st, keepSelection=false){
 ...
 const _ov = document.getElementById('firstRollPopupOverlay'); if(_ov) _ov.classList.add('bg-roll-popup');
...
function shareBgGame(){
 if(!bgCode)return;
 copyToClipboard(bgCode,{ru:'Код скопирован OK',uk:'Код скопійовано OK',en:'Code copied OK'}[lang]);
